...
function createNewNotebook() {
  ...
  firebase
    .database()
    .ref(userId + "/notebooks/" + notebookId)
    .set({
      notes: [],
      id: notebookId,
      name: name,
...
function deleteNotebook() {
  firebase
    .database()
    .ref(userId + "/notebooks/" + currentNotebookId)
    .remove();
...
function createNewNote() {
  var delta = {
    ops: [
      { attributes: { size: "huge" }, insert: "NEW NOTE...." },
      { insert: "\n" },
    ],
  };
  ...
  firebase
    .database()
    .ref(userId + "/notebooks/" + currentNotebookId + "/notes/" + currentNoteId)
    .set({
      rawData: delta,
      id: currentNoteId,
    });
...
function saveNote() {
  ...
  for (var i = 0; i < notesIds.length; i++) {
    if (notesIds[i] == currentNoteId) {
      userNotebooks[notebookSelect.selectedIndex].notes[
        notesIds[i]
      ].rawData = delta;
    }
  }
...
function deleteNote() {
  ...
  if (
    Object.keys(userNotebooks[notebookSelect.selectedIndex].notes).length <= 0
  ) {
    deleteNotebook();
  } else {
    for (var i = 0; i < notesIds.length; i++) {
      if (notesIds[i] == currentNoteId) {
        if (i != 0) {
          quill.setContents(
            userNotebooks[notebookSelect.selectedIndex].notes[notesIds[i - 1]]
              .rawData
          );
        } else {
          quill.setContents(
...
function openNote() {
  ...
  for (var i = 0; i < notesIds.length; i++) {
    if (userNotebooks[notebookSelect.selectedIndex].notes[aux].id == aux) {
      quill.setContents(
        userNotebooks[notebookSelect.selectedIndex].notes[aux].rawData
      );
      currentNoteId = userNotebooks[notebookSelect.selectedIndex].notes[aux].id;
    } else {
      quill.setContents(
        userNotebooks[notebookSelect.selectedIndex].notes[notesIds[0]].rawData
      );
      currentNoteId =
        userNotebooks[notebookSelect.selectedIndex].notes[notesIds[0]].id;
    }
  }
}
...
function getNotes() {
  ...
  for (let i = 0; i < notesIds.length; i++) {
    var el = document.createElement("option");
    var opt =
      userNotebooks[notebookSelect.selectedIndex].notes[notesIds[i]].rawData
        .ops[0].insert;
    el.textContent = opt;
    el.value =
      userNotebooks[notebookSelect.selectedIndex].notes[notesIds[i]].id;
    select.appendChild(el);
  }
  openNote();
}
...
function downloadFile() {
  ...
  var blob = new Blob([delta], { type: "text/plain;charset=utf-8" });
  saveAs(blob, select.options[select.selectedIndex].textContent + ".txt");
}

function moveNote() {
  var name = prompt(
    "Please the name of the notebook you want to transfer this note to:",
    "-"
  );
  for (var i = 0; i < userNotebooks.length; i++) {
    if (userNotebooks[i].name == name) {
      firebase
        .database()
        .ref(
          userId + "/notebooks/" + currentNotebookId + "/notes/" + currentNoteId
        )
        .remove();
      firebase
        .database()
        ...
        .set(userNotebooks[notebookSelect.selectedIndex].notes[currentNoteId]);
      userNotebooks[i].notes[currentNoteId] =
        userNotebooks[notebookSelect.selectedIndex].notes[currentNoteId];
      delete userNotebooks[notebookSelect.selectedIndex].notes[currentNoteId];
    }
  }
...
function originalTheme() {
  ...
  document.getElementById("logo").style.filter =
    "invert(0%) sepia(99%) saturate(2%) hue-rotate(310deg) brightness(109%) contrast(101%)";
}
...
function simpleWhiteTheme() {
  ...
  document.getElementById("logo").style.filter =
    "invert(100%) sepia(99%) saturate(2%) hue-rotate(310deg) brightness(109%) contrast(101%)";
}
...
function draculaTheme() {
  ...
  root.style.setProperty("--main-toolbar-color-contrast", "#FFF");
  document.getElementById("logo").style.filter =
    "invert(69%) sepia(37%) saturate(2176%) hue-rotate(289deg) brightness(102%) contrast(101%)";
}
...
function usaTheme() {
  ...
  root.style.setProperty("--main-toolbar-color-contrast", "#FFF");
  document.getElementById("logo").style.filter =
    "brightness(0) saturate(100%) invert(23%) sepia(9%) saturate(3324%) hue-rotate(202deg) brightness(93%) contrast(92%)";
}
...
function razzzrTheme() {
  ...
  root.style.setProperty("--main-toolbar-color-contrast", "#FFF");
  document.getElementById("logo").style.filter =
    "invert(64%) sepia(64%) saturate(4644%) hue-rotate(85deg) brightness(121%) contrast(125%)";
}
...
function coffeeBuzzTheme() {
  ...
  root.style.setProperty("--main-toolbar-color-contrast", "#FFF");
  document.getElementById("logo").style.filter =
    "invert(6%) sepia(65%) saturate(1608%) hue-rotate(334deg) brightness(83%) contrast(96%)";
}
